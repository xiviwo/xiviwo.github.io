...
In my test environment, our product ldap server is unaccessiable due to security issues. But, we need to verify ldap login for gitlab. So, we introduce a simple ldap server setup here.

Basically, there is already a ruby ldap server available [here][ldapserver], what we need to do is to configure and bring it up. here, it will be set up with chef recipe.
...
...
```
As test env, self-signed certificated will be generated by 
```console
...
schema is vital for ldap server to understand/recognize what consist of a ldap content. Here, we will continue utilize the `core.schema` from `ruby-ldapserver`, adding a few neccessary to our ldap server. You can/should add more according to your need.
...
systemd_unit 'ldap-server.service' do
  content(Unit: {
          ...
          Service: {
            Type: 'simple',
            Restart: 'always',
            ExecStart: '/bin/ruby /opt/ldap-server/bin/ldap_server.rb',
